...
""" ========================================
    sar_fastraw: SAR Raw data Generator (:mod:`srg`)
    ========================================
    ...
    **Arguments**
    ...
    The approach to speed up the process is to decompose the antenna pattern in a sum of patterns
    G(az) = \sum_i W_i(az) * G(az)
    with W_i rectangular (or triangular) windows. Each term in the sum will have a narrow Doppler bandwidth, meaning
    that we can simulate the scene at a lower PRF. At the end op the process we need to upsample each
    image add them all together. For the upsampling we need to make sure we that everything is at baseband,
    so we need to demodulate, upsample and remodulate. The gain is that we avoiding recomputing surfaces
    and surface scatterings often. The cost is an increased memory footprint.
...
if __name__ == '__main__':
    ...
    fastraw(args.cfg_file, args.output_file,
