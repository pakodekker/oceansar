    ...
    def t(self, value):
        ...
        if (t_l != self.t_l_last) or (t_h != self.t_h_last):
            ...
            for t_i in t_update:
                if self.rank == self.root:
                    ...
                    if 'D' in self.compute:
                        Dx_f = (self.surface.Dx,
                                (self.counts, self.displ), MPI.DOUBLE)
                ...
                if 'D' in self.compute:
                    self.comm.Scatterv(
                        Dx_f, (self._Dx[int(t_i[0])], MPI.DOUBLE), root=self.root)
                    self.comm.Scatterv(
                        Dy_f, (self._Dy[int(t_i[0])], MPI.DOUBLE), root=self.root)
                    self.comm.Scatterv(
                        Dz_f, (self._Dz[int(t_i[0])], MPI.DOUBLE), root=self.root)
                if 'Diff' in self.compute:
                    self.comm.Scatterv(
                        Diffx_f, (self._Diffx[int(t_i[0])], MPI.DOUBLE), root=self.root)
                    self.comm.Scatterv(
                        Diffy_f, (self._Diffy[int(t_i[0])], MPI.DOUBLE), root=self.root)
                if 'Diff2' in self.compute:
                    self.comm.Scatterv(
                        Diffxx_f, (self._Diffxx[int(t_i[0])], MPI.DOUBLE), root=self.root)
                    self.comm.Scatterv(
                        Diffyy_f, (self._Diffyy[int(t_i[0])], MPI.DOUBLE), root=self.root)
                    self.comm.Scatterv(
                        Diffxy_f, (self._Diffxy[int(t_i[0])], MPI.DOUBLE), root=self.root)
                if 'V' in self.compute:
                    self.comm.Scatterv(
                        Vx_f, (self._Vx[int(t_i[0])], MPI.DOUBLE), root=self.root)
                    self.comm.Scatterv(
                        Vy_f, (self._Vy[int(t_i[0])], MPI.DOUBLE), root=self.root)
                    self.comm.Scatterv(
                        Vz_f, (self._Vz[int(t_i[0])], MPI.DOUBLE), root=self.root)
                if 'A' in self.compute:
                    self.comm.Scatterv(
                        Ax_f, (self._Ax[int(t_i[0])], MPI.DOUBLE), root=self.root)
                    self.comm.Scatterv(
                        Ay_f, (self._Ay[int(t_i[0])], MPI.DOUBLE), root=self.root)
                    self.comm.Scatterv(
                        Az_f, (self._Az[int(t_i[0])], MPI.DOUBLE), root=self.root)
                if 'hMTF' in self.compute:
                    self.comm.Scatterv(
                        hMTF_f, (self._hMTF[int(t_i[0])], MPI.DOUBLE), root=self.root)
